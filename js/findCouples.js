/* В коде используется буфер для отслеживания правильно выбранных 
 * карточек, n за итерацию, чтобы не было возможности выбрать из еще раз. Другой подход: добавить объекту Image
 * поле chosen и проверять его без буфера. 
 * Данная идея пришпришла позже. К сожалению реализовал
 * через буфер. Переделывается за 10 минут.
 * Так же можно вешать обработчик клика на каждую ячейку и после его удалять. Идея не показалась здравой.
 * Правила: Например если n = 3, а выбрали две разные карточки (картинки не совпалисовпали)
 * то игроку не предалагается выбвыбрать 3ю, т к смысла в этом нет и итерация считается завершенной.
 */
